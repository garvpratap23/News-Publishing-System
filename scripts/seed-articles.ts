...
// Helper function to fetch REAL news from Perplexity API with full detailed content
async function fetchNewsFromAPI(category: string, query: string, limit: number): Promise<any[]> {
  ...
  try {
    console.log(`  📡 Fetching REAL news from Perplexity API for ${category}...`)
    const response = await fetch('https://api.perplexity.ai/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a professional news journalist. Provide accurate, real, current news stories with full details based on actual recent events.',
          },
          {
            role: 'user',
            content: `Write ${limit} real, current news articles about ${query}. Use actual recent news events. For each article:
...
5. Use professional journalism style like The Hindu or BBC

Format as a JSON array with fields: title, excerpt, fullContent (plain text paragraphs separated by \\n\\n)`,
          },
        ],
        temperature: 0.3,
        max_tokens: 8000,
    ...
    try {
      const newsItems = extractJSON(content)
      if (newsItems && newsItems.length > 0) {
        console.log(`  ✅ Fetched ${newsItems.length} REAL news articles`)
        // Add category and format dates
        return newsItems.map((item: any) => ({
          title: item.title,
          excerpt: item.excerpt,
          fullContent: item.fullContent,
          category: category,
          source: item.source || 'News Network',
          publishedAt: item.publishedAt || new Date().toISOString(),
        }))
      }
    } catch (parseError) {
      console.log('  ⚠️  JSON parse error, trying alternative extraction')
    }
...
async function seedArticles() {
  try {
    ...
    for (const category of categories) {
      ...
      // Fetch articles for each query in the category with retry logic
      for (const query of category.queries) {
        let fetchedArticles: any[] = []

        // Try up to 3 times with exponential backoff
        for (let attempt = 0; attempt < 3; attempt++) {
          fetchedArticles = await fetchNewsFromAPI(category.name, query, articlesPerQuery)
      ...
      for (let i = 0; i < categoryArticles.length; i++) {
        ...
        // Combine all paragraphs with images in journalism style
        const contentWithImages = `
          <p class="lead text-lg font-medium mb-4">${leadParagraph}</p>

          <figure class="my-8">
            <img src="https://picsum.photos/seed/${category.name.toLowerCase()}-${i}-main/1200/600" alt="${articleData.title || category.name + ' News'}" class="w-full rounded-sm" />
            <figcaption class="text-sm text-gray-600 mt-2 italic">${articleData.title || category.name + ' developments'} | Photo: News Network</figcaption>
          </figure>

          <p class="mb-4">${contextParagraphs[0]}</p>

          <p class="mb-4">${contextParagraphs[1]}</p>

          <p class="mb-4">${contextParagraphs[2]}</p>

          <figure class="my-8">
            <img src="https://picsum.photos/seed/${category.name.toLowerCase()}-${i}-detail/1200/600" alt="Additional context for ${category.name}" class="w-full rounded-sm" />
            <figcaption class="text-sm text-gray-600 mt-2 italic">Detailed view of ${category.name.toLowerCase()} developments | Photo: News Network</figcaption>
          </figure>

          <p class="mb-4">${analysisParagraphs[0]}</p>

          <p class="mb-4">${analysisParagraphs[1]}</p>

          <blockquote class="border-l-4 border-gray-800 pl-6 my-8 italic text-lg text-gray-700">
            ${analysisParagraphs[2]}
          </blockquote>

          <p class="mb-4">${implicationsParagraphs[0]}</p>

          <figure class="my-8">
            <img src="https://picsum.photos/seed/${category.name.toLowerCase()}-${i}-analysis/1200/600" alt="Analysis of ${category.name} situation" class="w-full rounded-sm" />
            <figcaption class="text-sm text-gray-600 mt-2 italic">Expert analysis and future outlook | Photo: News Network</figcaption>
          </figure>

          <p class="mb-4">${implicationsParagraphs[1]}</p>

          <p class="mb-4">${conclusionParagraph}</p>
        `
